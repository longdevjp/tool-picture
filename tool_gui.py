            ...
            for img_tag in images:
                ...
                try:
                    ...
                    if img_url and img_url not in seen_urls:
                        ...
                        # Lọc theo chế độ
                        if mode == 1 and w < 1280:
                            continue
                        elif mode == 2 and w < 1920:
                            continue

                        save_path = os.path.join(output_dir, f"{keyword.replace(' ', '_')}_{count+1}.jpg")
                        img.save(save_path)
                        count += 1
                        progress_callback(count, num_images)
                except:
                    continue
...
# CẮT VIDEO THEO SỐ GIÂY TUỲ CHỌN
...
# GUI
root = Tk()
root.title("📥 Tải ảnh + video (Cắt video theo giây)")
...
# Biến lưu lựa chọn
quality_all_var = IntVar(value=1)  # Mặc định chọn tải mọi loại
...
note_label = Label(root, text="📝 Video sẽ được cắt thành các đoạn đúng số giây bạn chọn", 
